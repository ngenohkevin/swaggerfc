...
export default function ShopPage() {
  return (
    <div className="bg-[#1a1f2e] text-white font-source min-h-screen">
      {/* Navigation */}
      <nav className="fixed top-0 w-full z-50 bg-[#1a1f2e]/95 backdrop-blur-sm border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <Link href="/design-1" className="flex items-center gap-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#c9a227] rounded-full flex items-center justify-center">
              <span className="font-oswald font-bold text-[#1a1f2e] text-lg sm:text-xl">SF</span>
            </div>
            <span className="font-oswald text-xl sm:text-2xl font-bold tracking-wide">SWAGGER FC</span>
          </Link>
          <Link href="/design-1" className="text-white/70 hover:text-[#c9a227] transition-colors text-sm font-medium">
            Back to Home
          </Link>
        </div>
      </nav>
      ...
      {/* Header */}
      <div className="pt-24 sm:pt-28 pb-8 sm:pb-12 bg-[#0f1219]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <span className="text-[#c9a227] font-oswald text-sm tracking-widest uppercase">Official Merchandise</span>
          <h1 className="font-oswald text-4xl sm:text-5xl md:text-6xl font-bold mt-2">CLUB SHOP</h1>
          <p className="text-white/60 mt-4 max-w-xl">
            Wear your colors with pride. Select your size and order via WhatsApp for easy delivery.
          </p>
        </div>
      </div>
...
function ProductCard({ product }: { product: typeof products[0] }) {
  ...
  return (
    <div className="bg-[#2d3548] rounded-xl overflow-hidden group">
      <div className="relative h-64 sm:h-72 overflow-hidden bg-[#1a1f2e]">
        <Image
          src={product.image}
          alt={product.name}
          fill
      ...
      <div className="p-5 sm:p-6">
        <h3 className="font-oswald text-xl font-bold">{product.name}</h3>
        <p className="text-white/50 text-sm mt-2 line-clamp-2">{product.description}</p>

        {/* Size Selection */}
        <div className="mt-4">
          <p className="text-white/70 text-sm mb-2">Select Size:</p>
              ...
              <button
                ...
                className={`px-3 py-1.5 text-sm font-medium rounded transition-all ${
                  selectedSize === size
                    ...
                    : "bg-[#1a1f2e] text-white/70 hover:bg-[#c9a227]/20 hover:text-[#c9a227]"
                }`}
              >
                {size}
              </button>
        ...
        <div className="flex items-center justify-between mt-6">
          <span className="font-oswald text-2xl font-bold text-[#c9a227]">KES {product.price}</span>
          <div className="relative">
            <a
              ...
              className={`px-4 py-2 font-medium flex items-center gap-2 transition-colors rounded ${
                selectedSize
            ...
            {showTooltip && (
              <div className="absolute bottom-full right-0 mb-2 px-3 py-2 bg-[#c9a227] text-[#1a1f2e] text-sm font-medium rounded shadow-lg whitespace-nowrap animate-pulse">
